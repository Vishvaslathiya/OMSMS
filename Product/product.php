                ...
                ?>
                    <section id="Projects" class="w-fit mx-auto grid grid-cols-3 lg:grid-cols-4 md:grid-cols-2 justify-items-center justify-center gap-y-20 gap-x-14 mt-10 mb-5">
                        <?php while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            $brand_name_result = mysqli_query($conn, $get_brand_name);
            ...
            <div class="flex w-full justify-center ">
                <ul class="flex items-center border border-[#e4e4e4]  bg-white p-4 rounded-xl">
                    ...
                    if ($page_no != 1) {
                        ...
                        echo "<a href='product.php?page=" . ($page_no - 1) . "' class=' w-9 h-9 flex items-center justify-center rounded-md border border-[#EDEFF1] text-[#838995] text-base hover:bg-primary hover:border-primary hover:text-white'>
                        <span>
                    ...
                    for ($i = 1; $i <= $count; $i++) {
                        if ($page_no != $i) {
                            echo "<li class='px-[6px]'>";
                            echo "<a href='product.php?page=$i' class=' w-9 h-9 flex items-center justify-center rounded-md border border-[#EDEFF1] text-[#838995] text-base hover:bg-primary hover:border-primary hover:text-white '> $i </a>";
                            echo "</li>";
                        }
                    }
                    if ($page_no != $count) {
