...
<body>
    ...
    // SQL query to get category names and product details
    $sql = "SELECT tp.CategoryName, p.prdName AS ProductName, SUM(o.PrdQty) AS SoldItems, SUM(o.PrdQty * p.prdPrice) AS TotalAmount
        FROM tblorders o
        JOIN tblprd p ON o.PrdId = p.ID
        JOIN tblprd tp ON tp.id = p.ID
        GROUP BY tp.CategoryName, o.PrdId
        ORDER BY tp.CategoryName, SoldItems DESC";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            if ($currentCategory !== $categoryName) {
                ...
                echo "<div class='row'>";
                echo "<div class='col-md-12 col-xl-3 d-flex flex-column justify-content-start'>";
                echo "<div class='ml-xl-4 mt-3'>";
                echo "<p class='card-title'>Detailed Reports</p>";
                echo "<h1 class='text-primary'>$sum</h1>";
                echo "<h3 class='font-weight-500 mb-xl-4 text-primary'>$categoryName</h3>";
                echo "<p class='mb-2 mb-xl-0'>The total number of orders in $categoryName's Product with numbers of orders in <span class='text-primary' style='font-size: 20px;'><b>" . date('F') . " month</b></span></p>";
                echo "</div></div>";

                $currentCategory = $categoryName;
            }

            echo "<div class='col-md-12 col-xl-9'>";
            echo "<div class='row'>";
            echo "<div class='col-md-6 border-right'>";
            echo "<div class='table-responsive mb-3 mb-md-0 mt-3'>";
            echo "<table class='table table-borderless report-table'>";
            echo "<tr><td class='text-muted'>$productName</td><td class='w-100 px-0'>";
            echo "<div class='progress progress-md mx-4'>";
            echo "<div class='progress-bar bg-primary' role='progressbar' style='width: $soldItems%' aria-valuenow='$soldItems' aria-valuemin='0' aria-valuemax='100'></div>";
            echo "</div></td><td><h5 class='font-weight-bold mb-0'>$soldItems</h5></td></tr>";
            echo "</table></div></div></div></div></div></div>"; // Closing divs for col-md-6, row, and carousel-item
        }
    } else {
