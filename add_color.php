                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form class="forms-sample" id="color_form" method="POST" enctype="multipart/form-data">
                                ...
                                <!-- buttons -->
                                <button type="submit" name="addcolor" id="addcolor" class="btn btn-primary mr-2">Add Color</button>
        ...
        $('#color_form').validate({
            ...
            messages: {
                'color_name': {
                    ...
                    number: "Please Enter Valid Color Name",
                },
            }
        })
    </script>
...
if (isset($_POST['addcolor'])) {
    ...
    $cid = "SELECT id FROM tblcolor WHERE name = '$color_name'";
    $cid_result = mysqli_query($con, $cid);
    if (mysqli_num_rows($cid_result) > 0) {
        echo "<script>toastr.error('Color Already Exists!')</script>";
    } else {
        $insert_color = "INSERT INTO tblcolor (name) VALUES ('$color_name')";
        $insert_color_result = mysqli_query($con, $insert_color);

        if ($insert_color_result) {
            echo "<script>alert('Color Added Successfully!'); location.href='other_details.php'</script>";
        } else {
